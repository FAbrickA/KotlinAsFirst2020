...
fun mergePhoneBooks(mapA: Map<String, String>, mapB: Map<String, String>): Map<String, String> {
    ...
    for (key in keys) {
        if (key in mapA) {
            if (key in mapB && mapA[key] != mapB[key]) map[key] = "${mapA[key]}, ${mapB[key]}"
            else map[key] = mapA[key]!!
        } else {
            map[key] = mapB[key]!!
        }
    }
...
fun canBuildFrom(chars: List<Char>, word: String): Boolean {
    ...
    val wordCharsSet = word.map { it.lowercase() }.toSet()
    for (wordChar in wordCharsSet) {
        if (wordChar !in charsSet) return false
    }
    return true
}
...
fun hasAnagrams(words: List<String>): Boolean {
    val sortedWordsList = words.map { it.lowercase().split("").sorted() }

    for (i in 0 until sortedWordsList.size - 1) {
        for (j in i + 1 until sortedWordsList.size) {
            if (sortedWordsList[i] == sortedWordsList[j]) return true
        }
    }
    return false
}
...
fun propagateHandshakes(friends: Map<String, Set<String>>): Map<String, Set<String>> {
    ...
    for (person in friends.keys) {
        val set = getSetOfFriends(friends, person)
        set.remove(person)
        map[person] = set.toSet()
        for (person2 in set) {
            if (person2 !in friends && person2 !in map) map[person2] = setOf()
        }
    }
    return map.toMap()
}
...
fun getSetOfFriends(
    ...
    if (person in set) return set // deny endless cycle
    set.add(person)

    if (person !in friends) return set
    for (friend in friends[person]!!) {
        if (friend !in set) getSetOfFriends(friends, friend, set)
    }
    return set
}
